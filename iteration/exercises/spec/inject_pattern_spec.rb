RSpec.describe 'inject pattern test' do
  ...
  it 'test 9' do
    ...
    final_grade = 0
    # Your code goes here
    grades.each do |category, num_array|
      final_grade += num_array.sum.to_f / num_array.size
    end
  ...
  it 'test 10' do
    ...
    # Your Code Here
    printable_menu = "Menu:\n"
    menu.each do |item, data|
      and_last = "and #{data[:flavors].last}"
      data[:flavors].pop
      data[:flavors]<< and_last
      data[:gluten_free] ? gluten = "gluten free" : gluten = "non gluten free"
      printable_menu += "- #{data[:flavors].join ", "} #{item} (#{gluten})\n"
    end
