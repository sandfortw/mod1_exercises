RSpec.describe 'group by pattern' do
  ...
  it 'groups by first letter' do
    ...
    words.each {|word| words_by_first_letter[word.chars[0]] << word}
  ...
  it 'group by number of zeroes' do
    #THE BELOW SEEMS REALLY HACKY. MAYBE THERE IS A BETTER WAY TO COUNT ZEROS. 
    numbers = [1, 3, 500, 200, 4000, 3000, 10000, 90, 20, 500000]
    grouped = Hash.new {|hash, key| hash[key] = []}
    numbers.each {|number| grouped[number.to_s.length - 1] << number }
  ...
  it 'group by order of magnitude' do
    ...
    grouped = Hash.new {|hash, key| hash[key] = []}
    numbers.each {|number| grouped[(number.to_s.length).abs] << number}
